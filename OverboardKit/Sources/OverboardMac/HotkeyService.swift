...
public extension KeyboardShortcuts.Name {
    ...
    static let toggleDrawer = Self("toggleDrawer", initial: .init(.v, modifiers: [.command, .shift]))

    /// Pastes (and pops) the next item from the paste stack.
    static let pasteNextFromStack = Self("pasteNextFromStack", initial: .init(.v, modifiers: [.command, .option]))

    /// Summons/dismisses the launcher bar. ⌥Space — ⌘Space belongs to Spotlight.
    static let toggleLauncher = Self("toggleLauncher", initial: .init(.space, modifiers: [.option]))
    ...
    static let toggleEmojiPicker = Self("toggleEmojiPicker", initial: .init(.space, modifiers: [.control, .command]))
}
