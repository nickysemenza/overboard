...
struct FileMetadataScannerTests {
    @Test func indexesMetadataAndReconcilesRenameAndDeletion() async throws {
        ...
        #expect(try await FileMetadataScanner.scan(root: root, exclusions: ["node_modules"], generation: "one", index: index).isEmpty)
        ...
        _ = try await FileMetadataScanner.scan(root: root, exclusions: ["node_modules"], generation: "two", index: index)
        #expect(try await index.search("hello").isEmpty)
        #expect(try await index.search("goodbye").count == 1)
        try FileManager.default.removeItem(at: renamed)
        _ = try await FileMetadataScanner.scan(root: root, exclusions: ["node_modules"], generation: "three", index: index)
    ...
    @Test func incrementalScanKeepsItsDirectoryAndReconcilesUnicodePaths() async throws {
        ...
        let issues = try await FileMetadataScanner.scan(root: root, exclusions: [], generation: "two", index: index, under: folder)
    ...
    @Test func excludesInternalsButAllowsExplicitCloudRoots() {
        ...
        #expect(!FileMetadataScanner.shouldInclude(home.appendingPathComponent("dev/.git/config"), root: home, exclusions: []))
        #expect(!FileMetadataScanner.shouldInclude(home.appendingPathComponent("private/report.pdf"), root: home, exclusions: [home.appendingPathComponent("private").path]))
    }
    ...
    @Test func missingLocationPreservesKnownFiles() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let index = try FileNameIndex()
        try await index.upsert([IndexedFile(path: root.appendingPathComponent("known.pdf").path, name: "known.pdf", root: root.path, generation: "old")])
